...
import { getBlockedUsers, unblockUser } from "../../api/auth.api";
...
export default function BlockedUsersSection() {
  ...
  const handleUnblock = async (user) => {
    setUnblocking({ ...unblocking, [user.id]: true });
    try {
      await unblockUser(user.username);
      setBlocked(blocked.filter(u => u.id !== user.id));
    } catch (err) {
      console.error("Failed to unblock:", err);
      alert(err.response?.data?.error || "Failed to unblock user");
    } finally {
      setUnblocking({ ...unblocking, [user.id]: false });
    }
  };
  ...
  return (
    <div>
      <h3 className="text-2xl font-bold text-white mb-6">Blocked Users</h3>
          ...
          {blocked.map((user) => (
            ...
            >
              ...
              {getAvatarUrl(user) ? (
                ...
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold opacity-50">
              ...
              <div className="flex-1">
                <div className="font-semibold text-gray-400">{user.username}</div>
                {user.bio && (
